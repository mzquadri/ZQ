...
Pipeline diagrams nearly always draw promotion as an arrow: train, evaluate, register, deploy. The arrow implies that a model which finished evaluation is a model that should ship.
...
<Callout title="A conjunction, not a sequence">
...
The gates are a floor. Without them the floor is wherever the last person's attention happened to stop.

## What the performance gate should measure
